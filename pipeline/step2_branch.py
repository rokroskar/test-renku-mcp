...
def main() -> None:
    ...
    parser.add_argument("--family", required=True, choices=list(MODELS))
    ...
    print(f"Branch '{args.family}' on {len(y_train):,} training images")

    estimators = {}
    entries = []
    for n_train in TRAIN_SIZES:
        key = f"{slug(args.family)}-{n_train}"
        x_fit, y_fit = subsample(x_train, y_train, n_train, SEED)

        started = time.perf_counter()
        estimator = MODELS[args.family](SEED)
        ...
        print(f"  {args.family} @ {len(y_fit):>6,}: fitted in {elapsed:.1f}s")

    joblib.dump(estimators, args.out / "estimators.joblib")
    (args.out / "branch.json").write_text(
        json.dumps(
            {"family": args.family, "seed": SEED, "configs": entries}, indent=2
        )
        + "\n"
    )
